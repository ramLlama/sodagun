...
## 2026-06-11 — PR #10 review follow-ups (in-guest dev loop)

- **`collect2: ld terminated with signal 9` is the OOM killer, not disk.** BFD ld loads all rlibs' debug info at once; linking this crate's debug test binary needs well over 3 GiB. Disk exhaustion says `No space left on device` instead.
- **Debian bookworm's `pre-commit` apt package (2.20) predates the `pre-push`/`pre-commit` stage names** (pre-commit ≥ 3.2) and rejects configs using them. Install a current one via `uv tool install pre-commit` (all-`language: system` hook configs need no hook envs, so the runner alone suffices).
- **In-guest `pre-commit install` is impossible under `data` git access AND unnecessary**: the hooks dir is a read-only host-code-execution surface by design, but it's *shared* — host-installed hooks fire in the guest. The generated hook script falls back to `command -v pre-commit` when the host's embedded interpreter path doesn't exist in the guest.
- **Global preflights kill guest-side dev loops.** The `msb --version` check in `main()` made every sodagun command fail in-guest, including pure-git ones. Scope environment checks to the command groups that need them.
- **"Green inner loop, gated push boundary" pattern**: environment-dependent tests self-skip with a stderr notice (virt probe: `/dev/kvm` / `kern.hv_support`), and a pre-push hook using the *same probe* refuses to push from hosts where they would have been skipped. The agent iterates green in the sandbox; the human pushes from a host where the tests really ran.
- **The SDK's msb resolution ladder is `MSB_PATH` env > SDK bridge > config > debug-build `<workspace>/build/msb` probe > `~/.microsandbox/bin` > PATH.** PATH is LAST: installing msb on PATH in a guest can be shadowed by the debug probe (a debug-built sodagun with cwd in the workspace resolves `/workspace/build/msb`, observed executing with empty `--version` output). Pin with `MSB_PATH` when provisioning msb in an image.
